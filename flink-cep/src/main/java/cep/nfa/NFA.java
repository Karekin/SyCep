...
import static cep.nfa.MigrationUtils.deserializeComputationStates;
...
import cep.nfa.sharedbuffer.EventId;
import cep.nfa.sharedbuffer.NodeId;
 ...
 *
 * <p>The {@link cep.operator.CepOperator CEP operator} keeps one NFA per key, for
...
public class NFA<T> {
    ...
    /**
     * The lengths of a windowed pattern, as specified using the  Pattern.within(Time,
     ...
     * cep.pattern.Pattern#within(Time) Pattern.within(Time)} method.
     */
...
//    private final long intervalWindowTime = ConfigManager.getIntervalWindowTime(); // 从配置加载
    ...
    private Collection<Map<String, List<T>>> doProcess(
        ...
        // 遍历之前所有匹配的事件
        for (ComputationState computationState : nfaState.getPartialMatches()) {
            // 判断新事件Event匹配之前已经匹配过的哪个状态，并为其版本号+1
    ...
    public static class MigratedNFA<T> {
        ...
        private final cep.nfa.SharedBuffer<T> sharedBuffer;

        public cep.nfa.SharedBuffer<T> getSharedBuffer() {
            return sharedBuffer;
        }
        ...
        MigratedNFA(
                ...
                final cep.nfa.SharedBuffer<T> sharedBuffer) {
    ...
    public static final class MigratedNFASerializerSnapshot<T>
        ...
        protected NFASerializer<T> createOuterSerializerWithNestedSerializers(
            ...
            @SuppressWarnings("unchecked")
            TypeSerializer<cep.nfa.SharedBuffer<T>> sharedBufferSerializer =
                    (TypeSerializer<cep.nfa.SharedBuffer<T>>) nestedSerializers[1];
    ...
    public static class NFASerializer<T> extends TypeSerializer<MigratedNFA<T>> {
        ...
        private final TypeSerializer<cep.nfa.SharedBuffer<T>>
        ...
        public NFASerializer(TypeSerializer<T> typeSerializer) {
            this(
                    typeSerializer,
                    new cep.nfa.SharedBuffer.SharedBufferSerializer<>(
        ...
        public MigratedNFA<T> deserialize(DataInputView source) throws IOException {
            ...
            cep.nfa.SharedBuffer<T> sharedBuffer =
